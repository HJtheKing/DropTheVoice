...
const checkDuplicate = async () => {
  try {
    const response = await axios.post('http://localhost:8080/api-user/check-duplicate', { userName: userId.value });
  ...
  if (form.value.validate()) {
    try {
      const response = await axios.post('http://localhost:8080/api-user/register', {
        userEmail: `${email.value}@${emailDomain.value === '직접 입력' ? customEmailDomain.value : emailDomain.value}`,
